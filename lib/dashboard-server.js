  ...
  if (request.method === 'POST' && pathname === '/api/sync-daily') {
    return authenticate(request, response, db, async () => {
      try {
        ...
        const result = await runSyncDaily({ db, date: targetDate });
...
async function readDashboard(requestUrl, response, db, operatorId, operator) {
  ...
  const hasOperatorFilter = Boolean(operatorId);
  ...
  const userList = uniqueUsers.sort((left, right) =>
    String(left.full_name || left.company_email || left.applywizz_id || left.telegram_chat_id || left.client_id).localeCompare(
      String(right.full_name || right.company_email || right.applywizz_id || right.telegram_chat_id || right.client_id)
    )
  );
