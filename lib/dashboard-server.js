...
async function sendOperatorOTPEmail(email, otp) {
  const fromEmail = process.env.FROM_EMAIL || process.env.SENDGRID_FROM_EMAIL || 'no-reply@example.com';
  if (!process.env.SENDGRID_API_KEY) {
...
async function requestOtp(request, response, db) {
  ...
  try {
    ...
    return sendJson(response, 500, { error: 'Failed to send OTP email.' });
  }
